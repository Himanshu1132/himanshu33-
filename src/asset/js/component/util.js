...
var Util = function() {
    ...
    return {
        ...
        animateClass: function(el, animationName, callback) {
            ...
            Util.addClass(el, 'animated ' + animationName);

            Util.one(el, animation, function() {
                Util.removeClass(el, 'animated ' + animationName);
            });
        ...
        isRTL: function() {
            var html = Util.getByTagName('html')[0];

            if (html) {
                return (Util.attr(html, 'direction') == 'rtl');
            }
        },
        ...
        scrollInit: function(element, options) {
            ...
            options = Util.deepExtend({}, pluginDefOptions, options);
            ...
            function init() {
                    ...
                    attrs.forEach(function(attrName) {
            			...
            			if ((/^data-.*/g).test(attrName)) {
                            if (['scroll', 'height', 'mobile-height'].includes(optionName) == false) {
                                ...
                                options[optionName] = Util.filterBoolean(element.getAttribute(attrName));
                            }
            			}
            		});
                }
                ...
                if (options.height instanceof Function) {
                    ...
                    if (Util.isMobileDevice() === true && options.mobileHeight) {
                ...
                if ((options.mobileNativeScroll || options.disableForMobile) && Util.isMobileDevice() === true) {
                    ps = Util.data(element).get('ps');
                    if (ps) {
                        if (options.resetHeightOnDestroy) {
                            Util.css(element, 'height', 'auto');
                        } else {
                            Util.css(element, 'overflow', 'auto');
                            if (height > 0) {
                                Util.css(element, 'height', height + 'px');
                            }
                        }

                        ps.destroy();
                        ps = Util.data(element).remove('ps');
                    } else if (height > 0){
                        Util.css(element, 'overflow', 'auto');
                        Util.css(element, 'height', height + 'px');
                    }
                ...
                if (Util.attr(element, 'data-window-scroll') == 'true') {
                ...
                if (ps) {
                    ...
                    Util.css(element, 'overflow', 'hidden');
                    Util.addClass(element, 'scroll');

                    ps = new PerfectScrollbar(element, options);

                    Util.data(element).set('ps', ps);
                }

                // Remember scroll position in cookie
                var uid = Util.attr(element, 'id');
                // Consider using Localstorage
                //if (options.rememberPosition === true && Cookies && uid) {
                //    if (Cookie.getCookie(uid)) {
                //        var pos = parseInt(Cookie.getCookie(uid));
                //
                //        if (pos > 0) {
                ...
                //        Cookie.setCookie(uid, element.scrollTop);
                //    });
                //}
            }
        ...
        scrollUpdate: function(element) {
            var ps = Util.data(element).get('ps');
            if (ps) {
                ps.update();
            }
        },
        ...
        scrollUpdateAll: function(parent) {
            var scrollers = Util.findAll(parent, '.ps');
            for (var i = 0, len = scrollers.length; i < len; i++) {
                Util.scrollUpdate(scrollers[i]);
            }
        },
        ...
        btnWait: function(el, cls, message, disable = true) {
            ...
            if (disable) {
                Util.attr(el, "disabled", true);
            }

            if (cls) {
                Util.addClass(el, cls);
                Util.attr(el, "wait-class", cls);
            }

            if (message) {
                var caption = Util.find(el, '.btn-caption');

                if (caption) {
                    Util.data(caption).set('caption', Util.getHTML(caption));
                    Util.setHTML(caption, message);
                } else {
                    Util.data(el).set('caption', Util.getHTML(el));
                    Util.setHTML(el, message);
                }
            }
        },
        ...
        btnRelease: function(el) {
            ...
            Util.removeAttr(el, "disabled");

            if (Util.hasAttr(el, "wait-class")) {
                Util.removeClass(el, Util.attr(el, "wait-class"));
            }

            var caption = Util.find(el, '.btn-caption');

            if (caption && Util.data(caption).has('caption')) {
                Util.setHTML(caption, Util.data(caption).get('caption'));
            } else if (Util.data(el).has('caption')) {
                Util.setHTML(el, Util.data(el).get('caption'));
            }
        },

        isOffscreen: function(el, direction, offset = 0) {
            var windowWidth = Util.getViewPort().width;
            var windowHeight = Util.getViewPort().height;

            var top = Util.offset(el).top;
            var height = Util.outerHeight(el) + offset;
            var left = Util.offset(el).left;
            var width = Util.outerWidth(el) + offset;
...
// Initialize Util class on document ready
Util.ready(function() {
	if (typeof AppSettings !== 'undefined') {
		Util.init(AppSettings);
	} else {
		Util.init();
	}
});
...
window.onload = function() {
    var result = Util.getByTagName('body');
    if (result && result[0]) {
        Util.removeClass(result[0], 'page-loading');
    }
}
